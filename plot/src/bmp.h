...
public:
    ...
    RGB(const svg::ColorBase& c) : 
        b_(std::get<2>(c.rgb())), g_(std::get<1>(c.rgb())), r_(std::get<0>(c.rgb())) {}
    RGB(const std::string& s) : RGB(color_from_string(s)) {}
    RGB(const char* s) : RGB(std::string(s)) {}
};
